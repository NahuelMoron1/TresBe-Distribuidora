...
export class OptionsService {
  ...
  async getProductOptionsTC(productID: string) {
    try {
      ...
      return this.errorService.handleError(
        error,
        'Error leyendo opciones de producto'
      );
    }
  }
  ...
  async getProductOptionsByNameTC(id: string) {
    if (!id) {
      return this.errorService.handleError(
        undefined,
        'No hay opciones de este producto por el momento'
      );
    }
    try {
      ...
      return this.errorService.handleError(
        error,
        'Error leyendo opciones de producto por nombre'
      );
    }
  }
  async deleteOneOption(featureID: string) {
    try {
      await this.deleteOption(featureID).toPromise();
      ...
      return this.errorService.handleError(
        error,
        'Error eliminando opcion de producto'
      );
    }
  }
  ...
  async createOption(featureAux: Options) {
    try {
      await this.saveOptions(featureAux).toPromise();
      ...
      return this.errorService.handleError(
        error,
        'Error agregando nueva opcion de producto'
      );
    }
  }
  async updateOneOption(index: number, featureAux: Options, oldID: string) {
    ...
    try {
      (
        await this.pricesService.updateOptionID(oldID, featureAux.id)
      ).toPromise();
      ...
      return this.errorService.handleError(
        error,
        'Error modificando opcion de producto'
      );
    }
  }
