...
export class ProductDataComponent implements OnInit {
  ...
  getItemsProduct() {
    ...
    if (name.length == 0 || image.length == 0 || this.category.length == 0) {
  ...
  getItemsPrice(optionID: string, toDo: string) {
    //Lee los input de la lista de precios del producto, crea una nueva lista de precios y la retorna
    if (toDo == 'add') {
      this.pricesID = this.generateRandomId(16);
      let costPrice = this.getNumber('costPriceInp');
      if (costPrice > 0) {
        let pricelist1 = costPrice * 1.35;
        let pricelist2 = costPrice * 1.35;
        let pricelistG = costPrice * 1.35;
        if (this.brand.toLowerCase() === 'tel') {
          pricelist1 = costPrice * 1.29;
          pricelist2 = costPrice * 1.35;
          pricelistG = costPrice * 1.22;
        }
        let pricelist3 = costPrice * 1.5;
        let pricelist4 = costPrice * 1.7;
        let pricelistE = costPrice * 1.16;

        let pricesAux: PriceXproduct = new PriceXproduct(
          this.pricesID,
          optionID,
          pricelist1,
          pricelist2,
      ...
      if (costPrice > 0) {
        ...
        let pricelist2 = costPrice * 1.35;
        if (this.brand.toLowerCase() !== 'tel') {
          pricelistG = costPrice * 1.35;
          pricelist1 = costPrice * 1.35;
        }
        let pricelist3 = costPrice * 1.5;
        let pricelist4 = costPrice * 1.7;
        let pricesAux: PriceXproduct = new PriceXproduct(
  ...
  returnPrice(optionID: string) {
    let optionPrice: PriceXproduct = new PriceXproduct(
      '',
      '',
      0,
      0,
  ...
  async setProductPrice(orderID: string, priceList: number) {
    ...
    let priceAux: PriceXproduct = new PriceXproduct(
      '',
      '',
      0,
      0,
  ...
  async modifyOptions(optionAux: Options, index: number) {
    if (this.options.length > 0) {
      ...
      if (optionName.length > 0) {
        ...
        let pricesAux: PriceXproduct | null = this.getItemsPrice(
          optionOldID,
          'update'
